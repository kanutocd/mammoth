...
class SlotInvalidationRecoveryServlet < WEBrick::HTTPServlet::AbstractServlet
...
server.mount "/webhook", SlotInvalidationRecoveryServlet
puts "slot invalidation recovery receiver listening on /webhook port=#{server.config[:Port]} data_dir=#{DATA_DIR}"
