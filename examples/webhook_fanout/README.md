...
This example demonstrates the `destinations` configuration shape for
...
- destination-scoped masking of `customer_email`
...
The audit destination masks `customer_email` in `data`, `identity`, and
`changes` without changing the primary destination. See
